...
function postCreate() {
	...
	addChildAt(background = new Bitmap(Assets.getBitmapData(Paths.image("game/soundTray/volumebox"))), index);
	_defaultScale = 0.6;
	screenCenter();
}

function postUpdate(elapsed:Float) {
	y = trueY = CoolUtil.fpsLerp(trueY, lerpYPos, 0);
	alpha = CoolUtil.fpsLerp(alpha, alphaTarget, 0);
...
function regenerateBars(event) {
	...
	{
		var bar:Bitmap;
		addChild(bar = new Bitmap(Assets.getBitmapData(Paths.image("game/soundTray/bar_" + (i + 1)))));
