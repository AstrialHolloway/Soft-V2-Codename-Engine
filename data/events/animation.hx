...
animationBG.color = 0xffffff;
animationBG.updateHitbox();
add(animationBG);

var animationSprite:FunkinSprite = new FunkinSprite(0, 0); // Declare a variable to store the text object
animationSprite.scale.set(2.4,2.4);
animationSprite.frames = Paths.getSparrowAtlas("stages/rolling girl/fullscreen/start");
animationSprite.animation.addByPrefix("1", "s1", 24, true);
animationSprite.animation.play("1");
animationSprite.updateHitbox();
add(animationSprite);

function onEvent(animation) {
  if (animation.event.name == "animation") {
